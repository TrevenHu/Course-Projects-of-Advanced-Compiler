...
class DataFlowAnalysis {
		...
		void initializeBackwardMap(Function * func) {
            ...
            for (Function::iterator bi = func->begin(), e = func->end(); bi != e; ++bi) {
				...
				for (auto pi = pred_begin(block), pe = pred_end(block); pi != pe; ++pi) {
					...
					Instruction * src = (Instruction *)prev->getTerminator();
					Instruction * dst = firstInstr;
					// exchange the dst and src position. add to edgeinfo.
  ...
  public:
    ...
    void runWorklistAlgorithm(Function * func) {
    	...
    	for(auto it = InstrToIndex.begin(), end = InstrToIndex.end() ; it != end ; it++){
        	//add the instr's index to worklist
    	...
    	while(!worklist.empty()){
    		...
    		unsigned instr_index = worklist.back();
    		// remove the node from the worklist
    		worklist.pop_back();
    		// if it's the dummy node (nullptr) instr_index = 0
    		if(!instr_index) continue;

    		getIncomingEdges(instr_index,&incoming_edges);
    		getOutgoingEdges(instr_index,&outgoing_edges);

    		std::vector<Info*> info_out;

    		flowfunction(IndexToInstr[instr_index], incoming_edges, outgoing_edges, info_out);
    		
    		int length = info_out.size();
    		for(int i = 0; i < length; i++){
    			Info * new_info = new Info();
    			Edge out_edge = Edge(instr_index, outgoing_edges[i]);
    			//std::map<Edge, Info *> EdgeToInfo;
    			//let new_info := m(n.outgoing_edges[i]) U info_out[i];
    			Info::join(EdgeToInfo[out_edge],info_out[i],new_info);
    			//if(m(n.outgoing_edges[i])!= new_info])
    			if(!Info::equals(EdgeToInfo[out_edge], new_info)){
